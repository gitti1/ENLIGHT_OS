...
This report provides an update on Open Science (OS) across the ENLIGHT university network based on a survey that was conducted in February/March 2024. The survey was open from 6 February until 12 March 2024. 
...
The main areas and focus of the second survey were to

* Update the information of the previous survey on Open Science (OS), Open Access (OA) and research data management (RDM) activities, 

* Identify achievements based on the ENLIGHT RISE activities, and 

* Identify topics and priorities for continued collaboration on Open Science in ENLIGHT. 

The survey findings will be published in the ENLIGHT Open Science Status Quo & Opportunities #2 (deliverable D7.2) report. 
...
### Function to split values and create a table 
# does not work - error based on "=" sign
# create_tab <- function(data, var, var_fill){ 
#   tab <- data %>% select({{var}}, {{var_fill}}) %>% 
...
# "rdm_services"               "rdm_staff"                  "rdm_actions"        
# "fair_challenges" 


# RDM policies - data sharing
rdm_policy_data_sharing_tab_2024 <- os_survey_2024 %>% 
  select(Zeitstempel, rdm_policy_data_sharing) %>% 
  transform(rdm_policy_data_sharing = strsplit(rdm_policy_data_sharing, ",")) %>% 
  unnest(rdm_policy_data_sharing) %>% 
  transform(rdm_policy_data_sharing = str_trim(rdm_policy_data_sharing, side = "both")) %>% 
  as_tibble() %>% 
  filter(!is.na(rdm_policy_data_sharing)) %>% 
  mutate(rdm_policy_data_sharing = factor(rdm_policy_data_sharing)) %>% 
  filter(!(rdm_policy_data_sharing %in% c("RDP is only under construction at our university", 'but we are thinking about "as open as possible..." principle', "No policy yet :-(", "no policy", "The data repository project and the research data policy are expected to become a reality in the next 2 years."))) %>% 
  mutate(rdm_policy_data_sharing = recode_factor(rdm_policy_data_sharing,
'The "as open as possible as closed as necessary" principle is promoted.' = 'The "as open as possible as closed \n as necessary" principle is promoted.')) 

standard_plot(rdm_policy_data_sharing_tab_2024, rdm_policy_data_sharing, title = "Promotion of data sharing at ENLIGHT partner universities", subtitle = "5 institutions with RDM policy in place", caption = "") + scale_y_continuous(limits = c(0, 4), breaks = c(0, 2, 4))


# RDM policies - FAIR data
rdm_policy_fair_tab_2024 <- os_survey_2024 %>% 
  select(Zeitstempel, rdm_policy_fair) %>% 
  transform(rdm_policy_fair = strsplit(rdm_policy_fair, ",")) %>% 
  unnest(rdm_policy_fair) %>% 
  transform(rdm_policy_fair = str_trim(rdm_policy_fair, side = "both")) %>% 
  as_tibble() %>% 
  filter(!is.na(rdm_policy_fair)) %>% 
  mutate(rdm_policy_fair = factor(rdm_policy_fair)) %>% 
  filter(!(rdm_policy_fair %in% c("no policy"))) %>% 
  mutate(rdm_policy_fair = recode_factor(rdm_policy_fair, "FAIR will be included in the policy under development." = "FAIR will be included in the policy \n under development.")) 

standard_plot(rdm_policy_fair_tab_2024, rdm_policy_fair, title = "Promotion of FAIR data at ENLIGHT partner universities", subtitle = "7 responses", caption = "") + scale_y_continuous(limits = c(0, 6), breaks = c(0, 2, 4, 6))
...
# RDM staff
rdm_staff_tab_2024 <- os_survey_2024 %>% 
  select(Zeitstempel, rdm_staff, policy_rdm_stand_alone) %>% 
  ...
  filter(!(rdm_staff %in% c("data curators)", "directly linked to or embedded in a department/project)", "data librarians", "is the university's own at the moment.", "The project to set up the data repository that we have just started includes the hiring of a data curator. The rest of the staff", "The university also partly rely on data stewards that are connected to national infrastructures/competence centers."))) %>% 
  mutate(rdm_staff = recode_factor(rdm_staff, "Data managers (e.g. research data managers" = "Data managers", "Infrastructure engineers and managers (e.g. data repository developer)" = " Infrastructure engineers \n and managers", "Dedicated data stewards (implementation and liaison role" = "Data stewards", "IT or librarian" = "IT or librarian, hiring of \n a data curator foreseen")) %>% 
  mutate(policy_rdm_stand_alone = factor(policy_rdm_stand_alone, labels = c("Not available, \n currently not considered", "Under discussion", "Under development", "Available & Updated since Jan 2022", "Available"), levels = c("Not available, currently not considered", "Under discussion", "Under development", "Available & Updated since January 2022", "Available")))

  
standard_plot3(rdm_staff_tab_2024, rdm_staff, fill = policy_rdm_stand_alone, title = "RDM staff at ENLIGHT partner universities \n (with policy information)", m = 10, subtitle = "responses", caption = "") + guides(fill=guide_legend(title="RDM policy"))


# rdm_actions    
# What actions are currently taken for fostering skills and capacities for RDM and making data FAIR? (Multiple choice possible)

rdm_actions_tab_2024 <- os_survey_2024 %>% 
  select(Zeitstempel, rdm_actions, policy_rdm_stand_alone) %>% 
  ...
  filter(!(rdm_actions %in% c("but on a limited level."))) %>% 
  mutate(rdm_actions = recode_factor(rdm_actions, "Training for researchers exist" = "Training sessions for researchers", "Training courses for PHD students" = "Training courses for PhD students", "Embedding data managers/stewards in projects" = "Embedding data managers/\nstewards in projects", "Engagement in working groups (e.g. RDA)" = "Engagement in working groups", "Training for data managers and data stewards" = "Training for data managers \n and data stewards")) %>% 
  mutate(policy_rdm_stand_alone = factor(policy_rdm_stand_alone, labels = c("Not available, \n currently not considered", "Under discussion", "Under development", "Available & Updated since Jan 2022", "Available"), levels = c("Not available, currently not considered", "Under discussion", "Under development", "Available & Updated since January 2022", "Available")))
  
standard_plot3(rdm_actions_tab_2024, rdm_actions, fill = policy_rdm_stand_alone, title = "Actions to foster RDM skills and capacity at ENLIGHT partner \n universities (with policy information)", m = 9, subtitle = "responses", caption = "") + guides(fill=guide_legend(title="RDM policy")) + scale_y_continuous(limits = c(0, 10), breaks = c(0, 5, 10))




# fair_challenges
os_survey_2024 %>% select(fair_challenges) %>% filter(!is.na(fair_challenges)) %>% unique()

```

As already noted above, 4 institutions have a RDM policy in place and for another four institution a policy is under development. The figure on RDM services covers information for both cases. For some figures we only consider those institutions who either have a policy in place or indicated their intent to include a certain measure (e.g. FAIR data).
